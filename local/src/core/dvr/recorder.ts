...
class CameraRecorder {
  ...
  start(): void {
    ...
    bobiStore.log('INFO', 'DVR', `Started recording: ${this.camera} camera`);
  ...
  stop(): void {
    ...
    if (this.currentSegment) {
      ...
      bobiStore.log('INFO', 'DVR', `Closed segment: ${this.currentSegment.id}`);
    }
    bobiStore.log('INFO', 'DVR', `Stopped recording: ${this.camera} camera`);
  }
  ...
  private startNewSegment(): void {
    ...
    bobiStore.log('DEBUG', 'DVR', `New segment: ${id}`, { camera: this.camera });

    // Rotate old segments (ring buffer)
    while (this.segments.length > ENV.DVR_MAX_SEGMENTS) {
      const removed = this.segments.shift();
      if (removed) {
        bobiStore.log('DEBUG', 'DVR', `Rotated out segment: ${removed.id}`);
  ...
  saveEventClip(eventId: string, beforeMs: number = 30000, afterMs: number = 10000): string {
    ...
    bobiStore.log('INFO', 'DVR', `Saving event clip: ${clipPath}`, { 
      eventId, 
      camera: this.camera,
      beforeMs,
      afterMs 
...
class DVRRecorder {
  ...
  startRecording(): void {
    ...
    bobiStore.log('INFO', 'DVR', 'DVR recording started (front + rear)');
  }

  stopRecording(): void {
    this.frontCamera.stop();
    this.rearCamera.stop();
    bobiStore.log('INFO', 'DVR', 'DVR recording stopped');
  }
